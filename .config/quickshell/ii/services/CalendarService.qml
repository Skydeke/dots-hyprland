...
Singleton {
        ...
        if (freq === 'WEEKLY') {
            ...
            while (weekStart <= untilDate && occurrences < maxOccurrences) {
                for (let wd of byDays) {
                    ...
                    // set time-of-day same as eventStart to keep consistent
    ...
    function getEventsInWeek() {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            const evs = root.getTasksByDate(d)
            result.push({
                name: days[i],
                events: evs.map(e => ({
                    title: e.content,
                    start: Qt.formatDateTime(e.startDate,"hh:mm"),
                    end: Qt.formatDateTime(e.endDate,"hh:mm"),
                    color: e.color,
                }))
            })
        }
        
        if (!root.edsAvailable && result.length > 0) {
            result[0].events.push({
                title: "Evolution Data Server unavailable",
                start: "00:00", end: "23:59", color:"#FF5555"
            })
        }
